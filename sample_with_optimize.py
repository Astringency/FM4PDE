...
from optimizing.optimizefunc import getPDEoptfunc
from data.transform import PDEtransform
from models.model_configs import instantiate_model
from sampling import generate_pde_withopt as generate_pde
...
parser.add_argument("--guide", type=str, default=True, help="Whether generate with guidence.")
...
parser.add_argument("--sampler", type=str, default="old", help="For the deterministic sampler, use new or old.")
parser.add_argument("--config", type=str, default=".", help="Config used to solve the specified PDE.")
parser.add_argument("--plot", type=bool, default=False, help="Whether to plot the generated samples.")
parser.add_argument("--pdeguide", type=bool, default=True, help="Whether to plot the generated samples.")
...
def progress_metrics(iters, coef_loss, sol_loss, pde_loss, params):
    table = Table.grid()
    table.add_row(f"[cyan]Epoch:[/] {iters}")
    table.add_row(f"[green]Loss(coef):[/] {coef_loss:.4f}")
    table.add_row(f"[green]Loss(sol):[/] {sol_loss:.4f}")
    table.add_row(f"[green]Loss(pde):[/] {pde_loss:.4f}")
    table.add_row(f"[green]Learned Parameters:[/] {params:.4f}")
...
def main(args, batch):
    ...
    get_opt_func = getPDEoptfunc(args.pdetype).func
    ...
    if config['data']['loadby'] == "scipy":
        ...
        data = {}
        with h5py.File(datapath, 'r') as file:
            # data[coef_name] = file[list(file.keys())[0]]['data']['h'][:] #type: ignore
            # data[sol_name] = file[list(file.keys())[0]]['data']['h'][:] #type: ignore
            # h0 = np.expand_dims(file[list(file.keys())[offset]]['data']['h'][:], axis = 0) # type: ignore
            # h = np.expand_dims(file[list(file.keys())[offset]]['data']['h'][:], axis = 0) # type: ignore
            # hu0 = np.expand_dims(file[list(file.keys())[offset]]['data']['hu'][:], axis = 0) # type: ignore
            # hu = np.expand_dims(file[list(file.keys())[offset]]['data']['hu'][:], axis = 0) # type: ignore
            # hv0 = np.expand_dims(file[list(file.keys())[offset]]['data']['hv'][:], axis = 0) # type: ignore
            # hv = np.expand_dims(file[list(file.keys())[offset]]['data']['hv'][:], axis = 0) # type: ignore
    ...
    if pde_type == "darcy":
        ...
        # a_GT = data[coef_name][offset, :, :, 0]
        a_GT = data[coef_name][0, :, :, :]
        a_GT = torch.tensor(a_GT, dtype=torch.float64, device=device)
        # u_GT = data[sol_name][offset, :, :, 0]
    ...
    # pde_inverse_transform = PDEtransform(data = data_GT, mode = "sample").inverse_transform_sample
    pde_inverse_transform = oldPDEtransform(pde_type).inverse_transform
    ...
    if a_GT.dim() == 3:
        ...
        if args.pdetype == "reaction_diffusion":
            a_GT = a_GT.permute(2, 0, 1)
            u_GT = u_GT.permute(2, 0, 1)
    else:
    ...
    # Learned_param
    learned_param = torch.nn.Parameter(get_opt_func())
    optimizer = torch.optim.AdamW([learned_param], lr=1e-4)
    ...
    with Live(refresh_per_second=10) as live:
        ...
        for i in range(1, len(T)):
            ...
            if i <= obsguide_t * num_steps:
                ...
                zeta_obs_a = config['generate']['zeta_obs_a'] * config['generate']['obsguide_decay']
                zeta_obs_u = config['generate']['zeta_obs_u'] * config['generate']['obsguide_decay']
                zeta_pde = config['generate']['zeta_pde']
            ...
            if config['generate']['samplemode'] == 'deterministic':
                ...
                x_1, x_N = deter_sampler(x_init, x_cur, t, step_size, method)
                x_tilde = (x_1 + 1.0) / 2.0
                x_next = (x_N + 1.0) / 2.0
                ...
                if i <= stochastic_t * num_steps:
                    ...
                    t = torch.tensor([T[i-1], T[i]])
                    x_1, x_N = deter_sampler(x_cur, t, step_size, method)
                    x_tilde = (x_1 + 1.0) / 2.0
                    x_next = (x_N + 1.0) / 2.0
            else:
            ...
            if img_channels == 1:
                ...
                a_N = x_tilde[:,0,:,:].unsqueeze(0)
                u_N = x_tilde[:,1,:,:].unsqueeze(0)
            elif img_channels == 4:
                a_N = x_tilde[:,:2,:,:].unsqueeze(0)
                u_N = x_tilde[:,2:,:,:].unsqueeze(0)
            else:
            ...
            pde_loss, observation_loss_a, observation_loss_u = get_pde_loss(a_N, u_N, a_GT, u_GT, known_index_a, known_index_u, learned_param, device=device)
            ...
            if img_channels == 1:
                ...
                a_eval = x_next[:,:2,:,:].unsqueeze(0)
                u_eval = x_next[:,2:,:,:].unsqueeze(0)
            else:
                print(">>> Unknown channels <<<")
                sys.exit()
            ...
            if args.pdetype == "shallow_water":
                re_a_eval = torch.norm(a_eval.squeeze() - a_GT.squeeze()[0], 2) / torch.norm(a_GT.squeeze()[0], 2)
                re_u_eval = torch.norm(u_eval.squeeze() - u_GT.squeeze()[0], 2) / torch.norm(u_GT.squeeze()[0], 2)
            else:
            ...
            live.update(
                    Group(
                        ...
                        progress_metrics(i, re_a_eval.item(), re_u_eval.item(), L_pde.item(), learned_param.item())
                        )
                    )
    ...
    if img_channels == 1:
        ...
        a_final = x_final[:,:2,:,:].unsqueeze(0)
        u_final = x_final[:,2:,:,:].unsqueeze(0)
    else:
        print(">>> Unknown channels <<<")
        sys.exit()
    ...
    # if pde_type == "darcy":
    #     a_final[a_final >= 7.5] = 12
    #     a_final[a_final < 7.5] = 3
    # else:
    #     a_final = a_final

    if args.pdetype == "shallow_water":
        relative_error_a = torch.norm(a_final.squeeze() - a_GT.squeeze()[0], 2) / torch.norm(a_GT.squeeze()[0], 2)
        relative_error_u = torch.norm(u_final.squeeze() - u_GT.squeeze()[0], 2) / torch.norm(u_GT.squeeze()[0], 2)
    else:
        relative_error_a = torch.norm(a_final.squeeze() - a_GT.squeeze(), 2) / torch.norm(a_GT.squeeze()[0], 2)
        relative_error_u = torch.norm(u_final.squeeze() - u_GT.squeeze(), 2) / torch.norm(u_GT.squeeze()[0], 2)

    print(f'Relative error of a: {relative_error_a}')
    print(f'Relative error of u: {relative_error_u}')
    print(f'Learned parameter: {learned_param.item()}')
    ...
    # Plot learning curve
    if args.plot:
        savedir = f'{config['output']['savepath']}/{cur_time}-FM4{pde_type}-obs{obs_size}-step({num_steps},{step_size})-zeta({config['generate']['zeta_obs_a']},{config['generate']['zeta_obs_u']},{config['generate']['zeta_pde']})-{method}-{config['generate']['samplemode']}-thre_t({stochastic_t},{obsguide_t})/{args.mode}-{args.problem}'
        ...
        plt.savefig(f'{savedir}/Generating_metrics.pdf')
        plt.savefig(f'{savedir}/Generating_metrics.eps')
        plt.close()
        ...
        #
        # plot_eval_pde(pde_type, data_GT, data_obs, data_final[0, :, :, :])
        # plt.savefig(f'{savedir}/FM4{pde_type}.pdf')
        # plt.savefig(f'{savedir}/FM4{pde_type}.eps')
        # plt.close()
    else:
    ...
    if config['output']['save']:
        # Save results
        with open(f'{config['output']['savepath']}{args.problem}/{config['data']['name']}_{args.mode}_{offset}_results.pkl', 'wb') as f:
            pickle.dump({
                'obs_index': {'known_index_a': known_index_a, 'known_index_u': known_index_u},
                'coef_final': a_final,
                'sol_final': u_final,
                'params': learned_param.item(),
                'loss': loss,
                'intermediate': sol,
                'time': time_eval,
                'config': config
