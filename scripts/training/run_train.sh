...
#   - data root: DATA_ROOT (default: datasets/)
#   - checkpoints/logs under outputs/pretrained/formal/<pde>/<timestamp...>/
#
# Examples:
#   bash scripts/training/run_train.sh
#   PDE=heat bash scripts/training/run_train.sh
#   PDE=heat SCALAR_CONDITIONING_PARAMS="alpha T" bash scripts/training/run_train.sh
#   PDE_LIST="heat wave nsnonbounded" NPROC_PER_NODE=2 bash scripts/training/run_train.sh
#   EPOCHS=300 TARGET_EFFECTIVE_BATCH=64 OUTPUT_DIR=outputs/pretrained/formal bash scripts/training/run_train.sh
#   MAX_TRAIN_SAMPLES=1024 DATA_SIZE=1 bash scripts/training/run_train.sh
#   PDE=heat TRAIN_DATA_CONFIG=configs/training_data.yaml bash scripts/training/run_train.sh
...
DATA_PATH="${DATA_PATH:-${DATA_ROOT:-${ROOT_DIR}/datasets}}"
DATA_PATH="${DATA_PATH%/}/"
...
TRAIN_DATA_CONFIG="${TRAIN_DATA_CONFIG:-configs/training_data.yaml}"
...
run_train() {
  ...
  model_profile="$(value_for_pde MODEL_PROFILE "${pde}" "${MODEL_PROFILE}")"
  if [[ "${pde}" == "nsnonbounded" && "${model_profile}" == "recommended" ]]; then
    model_profile=light
  fi
  ...
  if (( nproc > 1 )); then
    run_command "${PYTHON_BIN}" -m torch.distributed.run \
      ...
      --eval_frequency="${eval_frequency}" \
      "${extra_args[@]}" 2>&1 | { if [[ "${DRY_RUN}" == "true" ]]; then cat; else tee "${log_path}"; fi; }
  else
    run_command "${PYTHON_BIN}" -u train.py \
      --batch_size="${batch_size}" \
      --epochs="${EPOCHS}" \
      --accum_iter="${accum_iter}" \
      --dataset="${pde}" \
