...
#     TASK_LIST="forward inverse both" bash scripts/sampling/main/run_sample_sweep.sh
#   PARALLEL=true MAX_PARALLEL_TASKS=2 DEVICE_LIST="cuda:0 cuda:1" \
#     bash scripts/sampling/main/run_sample_sweep.sh
#   PDE_LIST="burger" TASK_LIST="both" \
#     SENSOR_MODE_LIST="random sensor_column" bash scripts/sampling/main/run_sample_sweep.sh
#   RESUME=true bash scripts/sampling/main/run_sample_sweep.sh
#
...
TASK_LIST="${TASK_LIST:-forward inverse both}" # forward inverse both
...
ARGS=(
    ...
    --sample-script "scripts/sampling/main/run_sample.sh"
    --pdes "${PDES[@]}"
    --tasks "${TASKS[@]}"
    --samplers "${SAMPLERS[@]}"
    --devices "${DEVICES[@]}"
...
exec "${PYTHON_BIN:-python}" -u -m sampling.sample_sweep "${ARGS[@]}"
