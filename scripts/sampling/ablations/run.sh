...
#   PDE_LIST="poisson heat" bash scripts/sampling/ablations/run.sh
#   PDE_LIST="poisson heat" bash scripts/sampling/ablations/run.sh \
#     guidance_components sampler_phase
#   PDE_LIST="poisson" PLAN_ONLY=true bash scripts/sampling/ablations/run.sh sampler_phase
...
GRID="${GRID:-configs/ablations/paper.yaml}"
...
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
    echo "Usage: bash scripts/sampling/ablations/run.sh [group ...]"
    echo ""
    ...
    echo '  PDE_LIST="poisson heat" PLAN_ONLY=true bash scripts/sampling/ablations/run.sh guidance_components'
    echo ""
    echo "Run tasks concurrently, for example:"
    echo '  PARALLEL=true MAX_PARALLEL_TASKS=2 DEVICE_LIST="cuda:0 cuda:1" bash scripts/sampling/ablations/run.sh'
    exit 0
fi
...
if is_true "${PLAN_ONLY}"; then
    exec "${PYTHON_BIN:-python}" -u -m sampling.sweep "${ARGS[@]}" --list
fi
if is_true "${DRY_RUN}"; then
    ARGS+=(--dry-run)
fi

"${PYTHON_BIN:-python}" -u -m sampling.sweep "${ARGS[@]}"

if is_true "${AGGREGATE}"; then
    "${PYTHON_BIN:-python}" -u -m sampling.aggregate "${OUTPUT_DIR}" --output-dir "${OUTPUT_DIR}"
fi
