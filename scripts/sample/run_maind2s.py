"""Run the existing main sweeps with S -> D at 0.8 and audit all 42 cells."""
...
def prepare(output, source):
    ...
    for pde, task in [(p, t) for p in PDES for t in TASKS] + [('burger', 'both')]:
        ...
        config.update(sampler_phase='hybrid_s2d', switch_ratio=0.8, checkpoint_path=str(checkpoint))
    ...
    manifest = dict(code_commit=subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip(),
                    ...
                    switch_ratio=0.8, num_steps=100, stochastic_steps=80, deterministic_steps=20,
    ...
    from sampling.sample_sweep import build_arg_parser, build_experiments
    rows = []
    for distribution in DISTRIBUTIONS:
        for burger in (False, True):
            args = build_arg_parser().parse_args([
                '--config-dir', str(output / 'configs'), '--output-dir', str(output / distribution),
                '--pdes', *(['burger'] if burger else PDES),
                '--tasks', *(['both'] if burger else TASKS),
                '--sensor-modes', *(['random', 'sensor_column'] if burger else ['random']),
                '--samplers', 'hybrid_s2d', '--num-steps', '100', '--num-obs', '500',
                '--sample-seed', '0', '--test-type', distribution, '--devices', 'cuda:0', 'cuda:1',
            ])
            experiments, _ = build_experiments(args)
            for experiment in experiments:
                rows.append(dict(test_type=distribution, pde=experiment.pde, task=experiment.task,
                    sensor_mode=experiment.sensor_mode, signature=experiment.signature,
                    config_path=str(experiment.config_path), resolved_config=experiment.config))
    assert len(rows) == 42 and all(r['resolved_config']['switch_ratio'] == .8 for r in rows)
    write(output / 'execution/resolved_matrix.json', dict(verified_cells=42, rows=rows))
    print('Prepared 42 cells, 42000 samples; S80 -> D20.', flush=True)
...
def validate_phases(root):
    ...
    for path in root.rglob('metrics_step.jsonl'):
        ...
        assert [r['phase'] for r in steps] == ['stochastic'] * 80 + ['deterministic'] * 20, path
        count += 1
    assert count, root
    return count
...
def audit(output):
    ...
    for case in cells():
        ...
        for config_path in root.rglob('resolved_config.yaml'):
            ...
            assert config['sampler_phase'] == 'hybrid_s2d' and config['switch_ratio'] == 0.8
    ...
    write(output / 'completion.json', dict(status='complete', verified_cells=42, verified_samples=42000,
          verified_runs=total_runs, sampler_phase='hybrid_s2d', switch_ratio=0.8, rows=summaries,
...
def main():
    ...
    if args.phase in ['full', 'all']:
        ...
        assert (args.workers // 2) * memory['pilot_batch_peak_mib'] < .9 * memory['gpu_total_mib']
        ...
        def run_distribution(distribution):
            sweep(args.output, distribution + '_main', distribution=distribution, batch=batch, workers=2)
            sweep(args.output, distribution + '_burger', distribution=distribution, batch=batch,
                  burger=True, aggregate=True, workers=2)
        with ThreadPoolExecutor(max_workers=args.workers // 2) as pool:
            futures = [pool.submit(run_distribution, distribution) for distribution in DISTRIBUTIONS]
