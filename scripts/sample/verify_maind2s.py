"""Independently verify saved maind2s S80+D20 batches, on the server or a local mirror."""
...
def verify(root, require_complete=False, check_inputs=False, check_tensors=False):
    ...
    for distribution in distributions:
        for marker_path in sorted((root / distribution).glob('.sample_sweeps/*/completed/**/*.json')):
            ...
            assert config['sampler_phase'] == 'hybrid_s2d' and config['switch_ratio'] == .8
            ...
            assert [step['phase'] for step in steps] == ['stochastic'] * 80 + ['deterministic'] * 20, folder
