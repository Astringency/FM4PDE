...
def main():
    ...
    protocol = dict(status="complete", groups=188, paired_comparisons=564,
        ...
        sample_selection="First 100 source samples, identical sample identities and physical observations across the four methods",
        physical_fields={"poisson": {"a": "forcing f", "u": "solution u"},
            "helmholtz": {"a": "forcing f", "u": "solution u"},
            "darcy": {"a": "coefficient a", "u": "solution u"},
            "nsnonbounded": {"a": "initial vorticity", "u": "final vorticity"}},
        observation_count="num_obs is the number of spatial positions per active field; both uses the same positions for its two fields",
        columns="Five full columns, 640 positions per active field, following the paper layout",
        missing_observations="Native variable observations and masked zero grids; interpolation uses only the retained real readings",
        average_comparison="FM4PDE uses the differentiable physical box-average operator; frozen baselines receive the same readings at window anchors through pretrained point-observation interfaces",
        average_point_control="average1 uses exactly the same interior anchors as average4, average8 and average16",
        training="No retraining or checkpoint changes; no task-specific hyperparameter search in this study",
        fm_sampling="100 stochastic steps, one archived seed-0 realization per sample, float32, TF32 disabled, batch size 16 and final 4-sample tail",
