...
默认使用 `trajectory_adjoint`、`force_steps=100` 的完整离散梯度。显式降低 `force_steps` 时，沿完整前向轨迹近似反向传播；`force_scale` 只校准提议。开发试验发现单独使用 4/20 步前向生成器的梯度偏差较大，已停止扩大这些配置。近似提议通过 MH 校正仍指向完整目标，但短链的混合可能不充分。
...
两张 GPU 可分别设置 `CUDA_VISIBLE_DEVICES=0/1`，使用相同参数、相同 `--name` 和 `--shard-count 2`，分别传 `--shard-index 0/1`。分片共享同一全量科学协议，结果批次互不覆盖；汇总程序仍要求两片合计全部 1000 个唯一输入。

全量运行不能只按显存可容纳的批量判断可行性。实测 32 条链的完整目标前向约 26.2 秒；若每输入 4 条链、64 次预热、128 次保留更新，用已测的 20 步独立前向梯度提议，两张 A100 约需 5.8 天。完整梯度会更贵；沿完整轨迹的近似提议可省下部分计算，但需要另行核查混合。此预算不表示 192 次更新足够收敛。
