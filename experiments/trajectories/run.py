...
def main():
 ...
 for pde in a.pdes:
  for mode,steps in [('pilot',100),('pilot',1000),('run',1000)]:
   name=f'{pde}_{mode}_{steps}';cmd=[sys.executable,str(pathlib.Path(__file__).with_name('collect.py')),'--root',str(a.root),'--diffusion-root',str(a.diffusion_root),'--pde',pde,'--mode',mode,'--steps',str(steps)]
