...
def main():
    ...
    p.add_argument("--diffusion-root", type=pathlib.Path, required=True)
    p.add_argument(
        "--pdes",
        nargs="+",
        default=["poisson", "darcy", "helmholtz", "burger", "nsnonbounded"],
    )
    a = p.parse_args()
    logs = a.root / "logs"
    ...
    for pde in a.pdes:
        for mode, steps in [("pilot", 100), ("pilot", 1000), ("run", 1000)]:
            name = f"{pde}_{mode}_{steps}"
            cmd = [
                sys.executable,
                str(pathlib.Path(__file__).with_name("collect.py")),
                "--root",
                str(a.root),
                "--diffusion-root",
                str(a.diffusion_root),
                "--pde",
                pde,
                "--mode",
                mode,
            ...
            (logs / (name + ".stage.json")).write_text(
                json.dumps(
                    {
                        "argv": cmd,
                        "started_unix": before,
