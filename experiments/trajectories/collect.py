...
def main():
    ...
    ap.add_argument("--mode", choices=["pilot", "run"], required=True)
    ap.add_argument("--steps", type=int, default=1000)
    ap.add_argument("--sample-id", type=int, action="append")
    ap.add_argument("--methods", nargs="+", default=["FM4PDE", "DiffusionPDE"])
    ap.add_argument("--diffusion-root", type=pathlib.Path, required=True)
    args = ap.parse_args()
    root = args.root.resolve()
    manifest = json.load(open(root / "manifest.json"))
    info = manifest["cells"][args.pde]
    inputs = root / info["input_root"]
    protocol = json.load(open(inputs / "protocol.json"))
    assert args.steps in [100, 1000]
    # Sources are immutable archived producer snapshots, not the current sampler.
    sys.path[:0] = [
        str(root / info["fm_code"]),
        str(root / info["fm_code"] / "plot"),
        str(args.diffusion_root),
    ]
    ...
    device = "cuda:0"
    out = root / ("pilots" if args.mode == "pilot" else "traces") / args.pde
    out.mkdir(parents=True, exist_ok=True)
    # Verify only this PDE's explicit inputs; shared source archives are hashed too.
    for item in manifest["artifacts"]:
        if (
            item["path"].startswith(info["input_root"] + "/")
            and "/original_timing/" not in item["path"]
        ):
            assert sha(root / item["path"]) == item["sha256"], item
    fm = load_fm4pde_checkpoint_bundle(
        str(root / info["fm_weights"]),
        args.pde,
        device,
        model_profile=protocol["fm_configs"][args.pde].get("model_profile", "auto"),
    )
    with open(root / info["diffusion_weights"], "rb") as f:
        dm = pickle.load(f)["ema"].to(device).eval()
    dm.requires_grad_(False)
    source = (root / info["diffusion_source"]).read_text()
    dm_plain, dm_trace, traced_source = diffusion_functions(source, torch, np)
    (out / "instrumented_diffusion.py").write_text(traced_source)
    ids = args.sample_id or (
        [info["pilot_id"]] if args.mode == "pilot" else info["evaluation_ids"]
    )
    allowed = [info["pilot_id"]] if args.mode == "pilot" else info["evaluation_ids"]
    assert all(i in allowed for i in ids)
    data = np.load(inputs / "truths.npz")
    mdata = np.load(inputs / "masks.npz")
    original_ids = protocol["evaluation_ids"] + [protocol["pilot_id"]]
    env = {
        ...
        "collector_sha256": sha(pathlib.Path(__file__)),
        "fm_commit": info["fm_commit"],
        "diffusion_commit": info["diffusion_commit"],
    }
    pilot_path = root / "pilots" / args.pde / f"certificate_{args.steps}.json"
    if args.mode == "run":
        certificate = json.load(open(pilot_path))
        assert certificate["status"] == "pass"
        assert certificate["environment"]["collector_sha256"] == env["collector_sha256"]
        assert certificate["environment"]["manifest_sha256"] == env["manifest_sha256"]
        assert certificate["environment"]["gpu"] == env["gpu"]
    spec = get_pde_spec(args.pde)
    checks = []
    for i in ids:
        j = original_ids.index(i)
        a = torch.from_numpy(data[args.pde + "_a"][j : j + 1]).to(device)
        u = torch.from_numpy(data[args.pde + "_u"][j : j + 1]).to(device)
        ma = torch.from_numpy(mdata[f"{args.pde}_{i}_a"])[None, None].to(device)
        mu = torch.from_numpy(mdata[f"{args.pde}_{i}_u"])[None, None].to(device)
        masks = PairMasks(ma, mu, {"source": "original frozen controlled timing masks"})
        ...
        def gt(aa, uu):
            return PDEGroundTruth(
                ...
                aa,
                uu,
                aa if args.pde == "burger" else torch.cat([aa, uu], 1),
                {},
                list(spec.coef_channel_names),
                list(spec.sol_channel_names),
                {
                    "sample_ids": [str(i)],
                    "sample_offsets": [i],
                    "offset": i,
                    "batch_size": 1,
                    "synthetic": False,
                    "endpoint_pair": args.pde != "burger",
                },
            )

        truth = gt(a, u)
        observed = gt(a * ma, u * mu)
        c = copy.deepcopy(protocol["fm_configs"][args.pde])
        c.update(
            device=device,
            num_steps=args.steps,
            batch_size=1,
            offset=i,
            sample_seed=protocol["seed"] + i,
            checkpoint_path=str(root / info["fm_weights"]),
            output_dir=str(out),
            initial_noise_source_indices=[],
        ...
        if args.pde == "burger":
            c.update(sensor_mode="sensor_column", num_sensor_columns=5, num_obs=640)
        cfg = AblationConfig(**c)
        cfg = r.finalize_ground_truth_config(cfg)
        cfg.validate()
        dc = copy.deepcopy(protocol["diffusion_configs"][args.pde])
        dc["generate"].update(device=device, seed=protocol["seed"] + i, batch_size=1)
        dc["test"]["iterations"] = args.steps
        for method in args.methods:
            dest = out / f"{method}_{args.steps}_{i}"
            receipt = dest.with_suffix(".json")
            if receipt.exists():
                old = json.load(open(receipt))
                assert old["environment"] == env
                assert sha(dest.with_suffix(".csv")) == old["trace_sha256"]
                print("SKIP", dest.name, flush=True)
                continue
            tracer = Trace(cfg, truth, masks, r, fm[1], torch)
            tracer.method = method

            def plain():
                return (
                    fm_predict(cfg, fm, observed, masks)
                    if method == "FM4PDE"
                    else dm_plain(
                        dc, dm, observed.coef, observed.sol, ma[0, 0], mu[0, 0]
                    )
                )
            ...
            tracer.begin()
            if method == "FM4PDE":
                original = r.sampler_step
                k = [0]

                def hook(*aa, **kk):
                    result = original(*aa, **kk)
                    ep = r._physical_from_model_state(
                        result.x_endpoint.detach(), cfg, fm[1]
                    )
                    nat = r._physical_from_model_state(
                        result.x_raw_current.detach(), cfg, fm[1]
                    )
                    tracer(
                        k[0], (ep.coef, ep.sol), (nat.coef, nat.sol), float(result.t)
                    )
                    k[0] += 1
                    return result
                ...
                try:
                    ...
                    r.sampler_step = original
            else:
                prediction = dm_trace(
                    dc, dm, observed.coef, observed.sol, ma[0, 0], mu[0, 0], tracer
                )
            tracer.final(prediction)
            torch.cuda.synchronize()
            wall = time.perf_counter() - tracer.start
            assert len(tracer.rows) == args.steps + 1
            assert [x["step"] for x in tracer.rows] == list(range(args.steps + 1))
            row = {
                "status": "complete",
                "pde": args.pde,
                "method": method,
                "sample_id": i,
                "steps": args.steps,
                "nfe": args.steps if method == "FM4PDE" else 2 * args.steps - 1,
                "environment": env,
                "effective_fm_config": dataclasses.asdict(cfg),
                "effective_diffusion_config": dc,
                "metric_residual_metadata": tracer.residual_meta,
                "wall_seconds_including_diagnostics": wall,
                "diagnostic_seconds": tracer.paused,
                "sampling_seconds_excluding_diagnostics": wall - tracer.paused,
                "peak_allocated_bytes": torch.cuda.max_memory_allocated(),
            }
            if baseline is not None:
                delta = max(
                    float((x - y).abs().max()) for x, y in zip(prediction, baseline)
                )
                row["uninstrumented_max_abs"] = delta
                assert delta == 0.0, (args.pde, method, delta)
                checks.append(
                    {
                        "method": method,
                        "sample_id": i,
                        "uninstrumented_max_abs": delta,
                        "finite": True,
                    }
                )
            oldpath = inputs / "original_timing" / f"{method}_{args.steps}_{i}.pt"
            if oldpath.exists():
                old = torch.load(oldpath, map_location=device, weights_only=False)
                row["historical_final_max_abs"] = max(
                    float((prediction[0] - old["coef"]).abs().max()),
                    float((prediction[1] - old["sol"]).abs().max()),
                )
                row["historical_final_relative_l2_difference"] = [
                    float((x - y).norm() / y.norm())
                    for x, y in zip(prediction, (old["coef"], old["sol"]))
                ]
            temp = dest.with_suffix(".partial.csv")
            with temp.open("w") as f:
                writer = csv.DictWriter(f, fieldnames=list(tracer.rows[0]))
                writer.writeheader()
                writer.writerows(tracer.rows)
            temp.rename(dest.with_suffix(".csv"))
            row["trace_sha256"] = sha(dest.with_suffix(".csv"))
            torch.save(
                {
                    "coef": prediction[0].detach().cpu(),
                    "sol": prediction[1].detach().cpu(),
                    "sample_id": i,
                    "input_manifest_sha256": env["manifest_sha256"],
                },
                dest.with_suffix(".pt"),
            )
            row["prediction_sha256"] = sha(dest.with_suffix(".pt"))
    ...
    if args.mode == "pilot":
        assert set(x["method"] for x in checks) == set(args.methods) or all(
            (out / f"{m}_{args.steps}_{ids[0]}.json").exists() for m in args.methods
        )
        write_json(
            pilot_path,
            {
                ...
                "steps": args.steps,
                "sample_ids": ids,
                "metric_semantics": "clean endpoint estimate; final index is final sample",
                "timing_note": manifest["timing_note"],
            },
        )
