...
def load_cell(
    input_root: str | Path, cell: dict[str, Any], *, verify_hashes: bool = True
) -> dict[str, Any]:
    ...
    for key in ["coef", "sol"]:
        ...
        if not torch.all(mask.flatten(1).sum(1) == 500):
            raise ValueError(
                "Stored sparse mask must contain exactly 500 points per field"
            )
    ...
    masks["metadata"] = dict(
        ...
        source_file=cell["masks_file"],
        source_sha256=cell["masks_sha256"],
    )
    return dict(
        coef_ground_truth=a,
        sol_ground_truth=u,
        masks=masks,
        sample_ids=list(truth["sample_ids"]),
        source_indices=truth["source_indices"],
        pde_params=dict(truth["pde_params"]),
