...
def code_identity():
    files = [Path(__file__), *sorted((ROOT / "sampling").glob("*.py")),
             *sorted((ROOT / "models").glob("*.py")),
             ROOT / "data" / "specs.py", ROOT / "data" / "transform.py"]
...
def runtime_environment(device, tf32):
    ...
    result = dict(host=socket.gethostname(), pid=os.getpid(), python=sys.version,
                  torch=torch.__version__, cuda=torch.version.cuda, device=str(d),
                  tf32=tf32, deterministic_algorithms=True,
                  visible_devices=os.environ.get("CUDA_VISIBLE_DEVICES"),
                  code_commit=subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=ROOT, text=True).strip())
    if d.type == "cuda":
        props = torch.cuda.get_device_properties(d)
        result.update(gpu=props.name, gpu_uuid=str(props.uuid),
...
def effective_config(cell, checkpoint, device, indices, count, output_dir=None):
    ...
    values.update(checkpoint_path=str(checkpoint), device=str(device),
                  output_dir=str(Path(output_dir or ROOT / "outputs" / TASK / "unwritten_kernel").resolve()),
                  batch_size=len(indices), offset=int(indices[0]),
                  initial_noise_source_batch_size=count,
                  initial_noise_source_indices=list(indices),
                  save_plots=False, save_intermediate=False,
                  save_per_sample_curves=False, empty_cache_each_step=False,
    ...
    if any(float(v or 0) != 0 for v in
           (cfg.noise_level, cfg.noise_level_coef, cfg.noise_level_sol)):
        raise ValueError("Noisy observations require a separately frozen observation payload")
    if cfg.obs_l2_reference_mse_zeta_a is not None or cfg.obs_l2_reference_mse_zeta_u is not None:
...
def observation_batch(data, cfg, indices, device):
    ...
    if bad:
        raise ValueError(f"Frozen main residual requires public scalar parameters only: {bad}")
    metadata = dict(sample_ids=[str(data["sample_ids"][i]) for i in indices],
                    sample_offsets=[int(data["source_indices"][i]) for i in indices],
                    offset=int(indices[0]), batch_size=len(indices), synthetic=False,
    ...
    hashes = {name: tensor_digest(tensor) for name, tensor in
              dict(coef_truth=coef, sol_truth=sol, coef_mask=ma, sol_mask=mu,
                   observed_coef=aa, observed_sol=uu).items()}
...
def infer(cfg, bundle, gt, masks, indices, *, steps=None, fused=True):
    ...
    classes, _ = r._class_conditioning_for_sampling(
        checkpoint_payload=payload, pde=cfg.pde, batch_size=len(indices),
        device=device, cfg_scale=cfg.cfg_scale)
    ...
    if isinstance(model, torch.nn.Module):
        ...
        if any(isinstance(m, torch.nn.modules.batchnorm._BatchNorm) for m in model.modules()):
            raise ValueError("Batch normalization is not permitted in this batching protocol")
    calls = [0]
    def count(*_):
        calls[0] += 1
    hook = model.register_forward_hook(count) if hasattr(model, "register_forward_hook") else None
    if device.type == "cuda":
    ...
    try:
        ...
        for step in range(actual_steps):
            ...
            phase = r.phase_for_step(cfg.sampler_phase, cfg.switch_ratio, step, cfg.num_steps)
            out = r.sampler_step(
                net, cur, t, tn, phase, cfg.step_method, cfg.loss_state,
                device=device, model_extra=extras,
        ...
        with torch.no_grad():
            ...
            pred = physical.sol if cfg.pde == "burger" else torch.cat([physical.coef, physical.sol], 1)
        if device.type == "cuda":
            torch.cuda.synchronize(device)
        ...
        if hook is not None and cfg.sampler_phase == "stochastic" and cfg.cfg_scale == 1 and calls[0] != expected_nfe:
            raise RuntimeError(f"Unexpected forward count {calls[0]} != {expected_nfe}")
        return pred.detach().cpu(), dict(
            seconds=seconds, nfe=calls[0] if hook else expected_nfe,
            steps=actual_steps, batch_size=len(indices), initial_noise_sha256=initial_hash,
...
def relative_differences(pred, reference):
    ...
    rel = torch.linalg.vector_norm(a - b, dim=1) / torch.linalg.vector_norm(b, dim=1).clamp_min(1e-12)
...
def score(pred, data, indices, cfg):
    ...
    def error(a, b):
        return torch.linalg.vector_norm((a - b).flatten(1), dim=1) / torch.linalg.vector_norm(b.flatten(1), dim=1).clamp_min(1e-12)
    ea, eu = error(pa, aa), error(pu, uu)
    return [dict(index=i, sample_id=str(data["sample_ids"][i]),
                 rel_l2_a=None if cfg.pde == "burger" else float(ea[j]),
                 rel_l2_u=float(eu[j])) for j, i in enumerate(indices)]


def certificate_identity(cell, bindings, environment, code):
    return dict(cell_id=cell["cell_id"], config_sha256=json_digest(cell["config"]),
                **bindings, code_sha256=json_digest(code),
                torch=environment["torch"], cuda=environment["cuda"],
                gpu=environment.get("gpu"), gpu_uuid=environment.get("gpu_uuid"),
                tf32=environment["tf32"], random_policy=RANDOM_POLICY)
...
def pilot_family(cell, bindings, environment, code):
    ...
    """
    keys = ["pde", "task", "guidance_components", "guidance_operator",
            "obs_guidance_reduction", "pde_guidance_reduction", "loss_state",
            "gradient_target", "sampler_phase", "step_method", "time_grid",
            "num_steps", "clip_mode", "residual_mode", "pde_residual_region",
            "boundary_condition_mode", "boundary_residual_normalization",
            "ns_operator_mode", "coef_positive_mode", "model_profile", "dtype",
            "img_channels", "img_resolution"]
    from sampling.config import AblationConfig
    c = AblationConfig(**cell["config"])
    family = {key: getattr(c, key) for key in keys}
    family.update(checkpoint_sha256=bindings["checkpoint_sha256"],
...
def pilot(args, cell, data, bundle, checkpoint, bindings, environment, code, folder):
    ...
    certificate = dict(status="pass", identity=identity,
                       ...
                       pilot_input_rows=ids, scope="Full 100-step numerical/memory pilot; these predictions are excluded from production aggregates")
...
def production(args, cell, data, bundle, checkpoint, bindings, environment, code, folder):
    import torch
    identity = certificate_identity(cell, bindings, environment, code)
    certificates = []
    ...
    family = pilot_family(cell, bindings, environment, code)
    matching = [p for p in matching if p.get("status") == "pass" and p.get("family") == family]
    if not any(p["max_batch_size"] >= args.batch_size for p in matching):
        raise ValueError("Production needs a matching successful PDE/task/model/operator/precision/GPU-model family pilot")
    count = len(data["sample_ids"])
    ...
    if todo:
        ...
        sanity_cfg = effective_config(cell, checkpoint, args.device, todo[:1], count, folder)
        sanity_gt, sanity_masks, _ = observation_batch(data, sanity_cfg, todo[:1], args.device)
    ...
    for start in range(0, len(todo), args.batch_size):
        ...
        payload = dict(cell_id=cell["cell_id"], indices=ids,
                       ...
                       **bindings, metrics=rows, inference=meta)
        torch.save(payload, temporary / "prediction.pt")
        with (temporary / "prediction.pt").open("rb") as stream:
            os.fsync(stream.fileno())
        receipt = dict(identity=identity, cell_id=cell["cell_id"], indices=ids,
                       sample_ids=payload["sample_ids"], source_indices=payload["source_indices"],
                       runtime_input_hashes=hashes, effective_config_sha256=json_digest(cfg.asdict()),
                       config_sha256=json_digest(cfg.asdict()),
                       observation_mask_source="frozen_external",
                       observation_masks_metadata=masks.metadata,
                       prediction_sha256=digest(temporary / "prediction.pt"),
                       metrics=rows, source_hashes=code, **bindings, **meta)
        ...
        try:
            ...
            os.close(directory)
        print("BATCH_COMPLETE", cell["cell_id"], ids[0], ids[-1],
              json.dumps(meta), flush=True)
...
def validate_protocol(protocol, mode):
    ...
    if protocol.get("status") not in allowed:
        raise ValueError(f"Protocol status {protocol.get('status')!r} is not allowed for mode={mode}")
    ...
    if any(type(cell.get("count")) is not int or cell["count"] != 1000 for cell in cells):
        raise ValueError("Each complete input cell must contain exactly 1000 rows")
    if mode == "run" and (len(cells) != 57 or sum(cell["count"] for cell in cells) != 57000):
        raise ValueError("Production requires the complete frozen 57-cell/57000-row protocol")
    return cell_map
...
def main(argv=None):
    ...
    if args.batch_size < 1 or not 0 < args.relative_tolerance <= .005:
        parser.error("Batch size must be positive; numerical tolerance must be in (0, .005]")
    ...
    with (job / "worker.lock").open("a+") as lock:
        ...
        write_json_new(job / ("invocation_" + uuid.uuid4().hex + ".json"),
                       dict(task=TASK, job_id=args.job_id, argv=sys.argv,
                            environment=environment, source_hashes=code,
                            protocol_sha256=digest(args.protocol)))
        from revision_pairing_0915.input_sources import load_cell
        ...
        for cell_id in args.cell:
            ...
            if loaded_checkpoint != (str(checkpoint), weight["sha256"]):
                ...
                bundle = load_fm4pde_checkpoint_bundle(
                    str(checkpoint), cell["pde"], args.device,
            ...
            bindings = dict(protocol_sha256=digest(args.protocol),
            ...
            operation(args, cell, data, bundle, checkpoint, bindings, environment, code, folder)
