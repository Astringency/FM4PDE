...
Each study retains a source-origin layer such as `server197/output`, `server193/output`, or `local_exports`, so identically named files from different sources are not overwritten. Resolve the authoritative input/result paths from the archive inventory before running a command. In the recipes, `$ABL`, `$NS_STUDY`, `$BURGERS`, `$OLD_NS`, and `$CHECKPOINT_STUDY` mean the appropriate restored study layout, not the parent directory above the origin layer. `$PAPER` is a separate working copy restored from `paper_assets_final_20260909` (the current tool default), containing the paper and frozen source data; `$REPLAY` is a fresh output directory. Run commands from the FM4PDE repository root with the environment recorded for the corresponding computation. For the CPU archive replays on server197, the tested interpreter is `/research_data/users/zhangxifeng/.conda/envs/fm4pde/bin/python`. The commands below assume that environment has been activated so `python` resolves to this path; the default server197 shell has no bare `python`. Alternatively, replace `python` with this full path. Historical inference and timing still require their own recorded environments. These variable names do not set system options or relocate files.
...
- `paper/source_data/main_figure_values.csv`: 264 final field/method/distribution rows for the five main tables, including the integrated new NS values. The pre-update sources remain in the separately preserved NS table-export audit.
...
- `paper/source_data/main_configs_archive.json.gz`: historical FM configurations and checkpoint paths. `main_hyperparameters_verified.csv` contains the final settings, including the separate new NS main model; do not interpret its NS rows as the old ablation checkpoint.
...
The approved `archive_inventory.conditional_scaling_final.json` has SHA `c473fa1979647f40144b515cbae49550b8b17edcdda6f628bbc29b98452f9ad9`. Its three entries total 14,385,940,996 bytes. Transfer started at 2026-09-09 18:37:55 +08:00 and all three entries completed independent verification at 19:38:49 +08:00. They are included in the final 67-entry ledger, `archive_completed_67_entries.json` (SHA `8f8e252898ce9bd7d9207136bf2225bc1412ee5bf36485821f6399adc9baabc0`). Preserve each independent copy/verify receipt when replaying from the destination. The target mapping is:
...
The checker recomputes both field errors and observation MSEs from all 480 physical means, then checks the twenty mean/SD/pointwise-interval summaries, sixteen paired intervals and fifteen latency summaries. It also compares every merged numerical row and all 672 arrays with the two complete host exports. Those manifests retain all 480 raw-result checksums for exactly 96 distinct task/input groups. Original absolute source-path strings are preserved as provenance; the command reads the explicit `--paper` and `--audit` roots and does not rewrite those records. Raw-pool identity, PDE residuals and stochastic-noise correspondence remain established by the upstream host audits, rather than being newly reconstructed by this compact-data check. The checker does not sample models or grant a new scientific/visual review. The prior one-pool path-regression receipt remains historical evidence. A subsequent complete destination replay has now passed: [K_ARCHIVED_RECOMPUTATION.md](K_ARCHIVED_RECOMPUTATION.md) gives the actual two-thread commands for all 96 raw pools and the 480-row compact checker. Both original host CSV/NPZ exports are byte-identical after re-export; 7,680 numerical values and 672 arrays match exactly. All 1,381 source files (14,385,940,996 bytes) retain their receipt hashes before and after. The initial multi-shard environment-list order assertion and its correction are retained explicitly; each original environment record is unchanged.
