...
def main(args):
    ...
    if args.external_receipts:
        for external in json.loads(args.external_receipts.read_text()):
            key = external['kind'], external['run']
            assert key not in prior and external['returncode'] == 0
            assert sha256(external['copied_receipt']) == external['copied_receipt_sha256']
            assert external['source_host'] == 'server193'
            prior[key] = external
    ...
    def run(record, kind, index):
        ...
        if key in prior:
            ...
            saved_receipt = Path(original.get('copied_receipt', prior_output/'replay_receipt.json'))
            ...
            row = dict(original, reused_receipt=str(saved_receipt), reused_receipt_sha256=sha256(saved_receipt),
                       reuse_reason='Identical wrapper and original source commit per cell, unchanged full cache hashes and original summary/config hashes. Individual receipts preserve their actual CPU environment and any original-host dependency boundary.')
...
if __name__ == '__main__':
    ...
    p.add_argument('--external-receipts', type=Path)
    main(p.parse_args())
