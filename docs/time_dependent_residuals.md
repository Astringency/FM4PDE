...
Near-endpoint temporal observations are not initial conditions. IC residuals are added only when `pde_params["observed_initial"]` or `pde_params["true_initial"]` is available, and `initial_mask` is honored for sparse forward observations.
...
`legacy_ignore_boundary: true` or `boundary_condition_mode: legacy_ignore` reproduces the old boundary-excluded residual behavior. It is intended only for old-result ablations, not formal results.
