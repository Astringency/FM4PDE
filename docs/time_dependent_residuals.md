...
Burgers is the only current full time-space tensor residual: the BCHW tensor itself stores a one-dimensional PDE time-space field, with `H` as time and `W` as space. It uses `u_t + u u_x - nu u_xx`, with centered finite differences in time and Fourier differentiation in periodic space, and records `status=reliable`.
...
Coarse two-endpoint residual used in the main Navier--Stokes comparisons and temporal ablations:
