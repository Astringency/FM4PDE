...
def aggregate_root(root: str | Path, output_dir: str | Path | None = None) -> dict[str, Path]:
    ...
    raw_rows, curve_rows = _collect_rows(root)
    ...
    _write_csv(grouped_path, _aggregate_rows(raw_rows, SUMMARY_METRICS, GROUP_KEYS))
    _write_csv(curves_path, _aggregate_rows(curve_rows, CURVE_METRICS, GROUP_KEYS + ["step"]))
    return {"raw": raw_path, "grouped": grouped_path, "curves": curves_path}


def _collect_rows(root: Path) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    raw_rows = []
    curve_rows = []
