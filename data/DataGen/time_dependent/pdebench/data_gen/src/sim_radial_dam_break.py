...
class Basic2DScenario(ABC):
    ...
    def save_state_to_disk(self, data_f, seed_str) -> None:
        ...
        V = np.expand_dims(np.asarray(self.save_state["v"]), -1)
        HU = np.expand_dims(np.asarray(self.save_state["hu"]), -1)
        HV = np.expand_dims(np.asarray(self.save_state["hv"]), -1)

        data_f.create_dataset(f"{seed_str}/data/h", data=H, dtype="f")
        data_f.create_dataset(f"{seed_str}/data/u", data=U, dtype="f")
        data_f.create_dataset(f"{seed_str}/data/v", data=V, dtype="f")
        data_f.create_dataset(f"{seed_str}/data/hu", data=HU, dtype="f")
        data_f.create_dataset(f"{seed_str}/data/hv", data=HV, dtype="f")
