...
INVERSE_TUNED_ZETAS = {
    "darcy": (0.0, 2_000_000_000.0, 0.03),
    "poisson": (0.0, 360_000_000.0, 0.3),
    "helmholtz": (0.0, 320_000_000.0, 0.03),
    "nsnonbounded": (0.0, 240_000.0, 0.3),
    "reaction_diffusion": (0.0, 4_000_000.0, 10.0),
    "shallow_water": (0.0, 50_000.0, 0.0),
    "heat": (0.0, 100_000.0, 0.0),
    "wave": (0.0, 1_000_000.0, 0.0),
    "advection_diffusion": (0.0, 1_000_000.0, 0.0),
    "steady_heat_conduction": (0.0, 10_000.0, 0.0),
}


def test_non_burger_main_configs_use_provisional_inverse_tuning():
    for pde, expected_zetas in INVERSE_TUNED_ZETAS.items():
        ...
        assert (config.zeta_obs_a, config.zeta_obs_u, config.zeta_pde) == expected_zetas
...
def test_main_config_tree_has_task_specific_files_and_burger_only_in_both():
    non_burger_pdes = set(INVERSE_TUNED_ZETAS)
