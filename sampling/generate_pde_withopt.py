...
class getPDEloss:
    def __init__(self, pde: str):
        self.pdes = {
            ...
            "shallow_water_opt": self.shallow_water_optimize,
    ...
    def helmholtz(self, a, u, a_GT, u_GT, a_mask, u_mask, k, device=torch.device('cuda')):
        """
    ...
    def shallow_water_optimize(self, a, u, a_GT, u_GT, a_mask, u_mask, device=torch.device('cuda')):
        # Reshape GT to (1, 3, 128, 128)
        a_GT = a_GT.reshape(1, 3, 128, 128)
        u_GT = u_GT.reshape(1, 3, 128, 128)
        ...
        # Observation losses for h (depth)
        observation_loss_a = (a - a_GT[:, 0:1, :, :]).squeeze()  # a is predicted h0
        observation_loss_a = observation_loss_a * a_mask  # apply mask

        observation_loss_u = (u - u_GT[:, 0:1, :, :]).squeeze()   # u is predicted h
        observation_loss_u = observation_loss_u * u_mask  # apply mask
