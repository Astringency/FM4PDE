...
from sampling.config import str2bool
from sampling.runner import run_from_legacy_args
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        choices=["deterministic", "stochastic", "hybrid_d2s", "hybrid_s2d"],
        help="Sampler phase.",
    )
...
def main() -> None:
    ...
    message = (
        "The legacy sampling CLI only maps old arguments into sampling.runner. "
        "Prefer `python -m sampling.runner --config ...` for new runs."
    )
