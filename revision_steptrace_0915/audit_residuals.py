...
def main():
    ...
    for method in ['FM4PDE', 'DiffusionPDE']:
        for sid in info['evaluation_ids']:
            ...
            prediction = torch.load(stem.with_suffix('.pt'), map_location='cpu', weights_only=False)
            ...
            gt = PDEGroundTruth(args.pde, a, u, a if args.pde == 'burger' else torch.cat([a,u],1),
                                {}, ['a'], ['u'], {'synthetic': False})
            ...
            cfg = AblationConfig(**{k:v for k,v in saved_config.items() if fields[k].init})
            ...
            with torch.no_grad():
                output = compute_guidance_losses(SplitState(prediction['coef'], prediction['sol']),
                                                   gt, PairMasks(ma, mu, {}), cfg)
            ...
            assert math.isfinite(value) and math.isfinite(saved) and math.isfinite(double_value)
            ...
            records.append({'method': method, 'sample_id': sid, 'saved_gpu_L_pde': saved,
                            'recomputed_cpu_L_pde': value, 'absolute_difference': delta,
    ...
    report = {'status': 'error' if failures else 'partial' if missing else 'pass',
              'pde': args.pde, 'manifest_sha256': sha(root/'manifest.json'),
              'operator_commit': info['fm_commit'],
              'operator_file_sha256': sha(root/info['fm_code']/'sampling/pde_residuals.py'),
              ...
              'comparison_relative_tolerance': 1e-3, 'comparison_absolute_tolerance': 1e-7,
              'records': records, 'missing': missing, 'failed_calls': len(failures)}
    out = root / 'audit'
    out.mkdir(exist_ok=True)
    (out/f'{args.pde}_final_residual_audit.json').write_text(json.dumps(report, indent=2)+'\n')
    print(args.pde, report['status'], len(records), 'failures', len(failures),
          'max_relative_difference', max((r['relative_difference'] for r in records), default=None))
    if failures:
        raise AssertionError(f'{len(failures)} independently recomputed PDE scores differ')
